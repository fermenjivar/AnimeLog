  ...
  if(isset($_POST['anime']) and isset($_POST['modificar'])){
      ...
      $score = $_POST['score'];
        if(isset($_POST['nombre']) and $nombre != ""){
            $consulta1 = mysql_query("UPDATE serie set nombre='$nombre'");
        }
        if(isset($_POST['year']) and $year != ""){
            $consulta2 = mysql_query("UPDATE serie set year='$year'");
        }
        if(isset($_POST['season']) and $season != ""){
            $consulta3 = mysql_query("UPDATE serie set season='$season'");
        }
        if(isset($_POST['score']) and $score != ""){
            $consulta4 = mysql_query("UPDATE serie set score='$score'");
        } 
    }   
  ?>
</form>
